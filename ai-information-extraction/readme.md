...
1.	In a web browser, open the Azure AI Foundry portal at https://ai.azure.com and sign in using the username User1-56982787@LODSPRODMCA.onmicrosoft.com and the password x=9&-=w=. Close any tips or quick start panes that are opened the first time you sign in, and if necessary use the Azure AI Foundry logo at the top left to navigate to the home page, which looks similar to the following image (close the Help pane if it's open):
...
o	Project: project56982787
o	Hub: hub56982787
o	Subscription: Your Azure subscription
o	Resource group: ResourceGroup1
o	Region: westus
...
1.	In a new browser tab, download content.zip from https://github.com/microsoftlearning/mslearn-ai-information-extraction/raw/main/Labfiles/content/content.zip and save it in a local folder.
...
o	Task name: Invoice analysis
o	Description: Extract data from an invoice
o	Single file content analysis: Selected
o	Advanced settings:
...
7.	In the list of suggested fields, select BillingAddress. This field is not needed for the invoice format you have uploaded, so use the Delete field (🗑) icon that appears in the selected field row to delete it.
8.	Now delete the following suggested fields, which aren't needed for your invoice schema:
o	BillingAddressRecipient
o	CustomerAddressRecipient
o	CustomerId
o	CustomerTaxId
o	DueDate
o	InvoiceTotal
o	PaymentTerm
o	PreviousUnpaidBalance
o	PurchaseOrder
o	RemittanceAddress
o	RemittanceAddressRecipient
o	ServiceAddress
o	ServiceAddressRecipient
o	ShippingAddress
o	ShippingAddressRecipient
o	TotalDiscount
o	VendorAddressRecipient
o	VendorTaxId
o	TaxDetails
...
10.	In the row for the Items field, note that this field is a table (it contains the collection of items in the invoice). Select it's Edit (▦) icon to open a new page with its subfields.
...
o	Name: invoice-analyzer
o	Description: Invoice analyzer
...
2.	On the Content Understanding page, select the Custom task tab at the top.
3.	On the Content Understanding custom task page, select + Create, and create a task with the following settings:
o	Task name: Slide analysis
o	Description: Extract data from an image of a slide
o	Single file content analysis: Selected
o	Advanced settings:
	Azure AI services connection: The Azure AI Services resource in your Azure AI Foundry hub
	Azure Blob Storage account: The default storage account in your Azure AI Foundry hub
...
1.	Select the Analyzer list page, and then select + Build analyzer and build a new analyzer with the following properties (typed exactly as shown here):
o	Name: slide-analyzer
o	Description: Slide image analyzer
2.	Wait for the new analyzer to be ready (use the Refresh button to check).
...
2.	On the Content Understanding page, select the Custom task tab at the top.
3.	On the Content Understanding custom task page, select + Create, and create a task with the following settings:
o	Task name: Voicemail analysis
o	Description: Extract data from a voicemail recording
o	Single file content analysis: Selected
o	Advanced settings:
	Azure AI services connection: The Azure AI Services resource in your Azure AI Foundry hub
	Azure Blob Storage account: The default storage account in your Azure AI Foundry hub
...
1.	Select the Analyzer list page, and then select + Build analyzer and build a new analyzer with the following properties (typed exactly as shown here):
o	Name: voicemail-analyzer
o	Description: Voicemail audio analyzer
2.	Wait for the new analyzer to be ready (use the Refresh button to check).
...
2.	On the Content Understanding page, select the Custom task tab at the top.
3.	On the Content Understanding custom task page, select + Create, and create a task with the following settings:
o	Task name: Conference call video analysis
o	Description: Extract data from a video conference recording
o	Single file content analysis: Selected
o	Advanced settings:
	Azure AI services connection: The Azure AI Services resource in your Azure AI Foundry hub
	Azure Blob Storage account: The default storage account in your Azure AI Foundry hub
...
1.	Select the Analyzer list page, and then select + Build analyzer and build a new analyzer with the following properties (typed exactly as shown here):
o	Name: conference-call-analyzer
o	Description: Conference call video analyzer
2.	Wait for the new analyzer to be ready (use the Refresh button to check).
